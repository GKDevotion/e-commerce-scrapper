@extends('layouts.app')
@section('title', 'AI Generated Listing')
@section('page-title', 'Listing Comparison')
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4 fade-in-up">
    <div>
        ...
        <h2 style="font-family:'Sora',sans-serif;font-size:18px;font-weight:700;color:#111827;margin:0;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            {{ Str::limit($generation->generated_title ?? 'Listing', 60) }}
            @if($generation->isManual())
    ...
    <div class="d-flex gap-2 flex-wrap">
        <form method="POST" action="{{ route('generations.favorite', $generation->id) }}" class="d-inline">
            @csrf
            <button type="submit" class="btn-alb-outline btn" style="{{ $generation->is_favorite ? 'color:#F59E0B;border-color:#F59E0B;' : '' }}">
                <i class="bi {{ $generation->is_favorite ? 'bi-star-fill' : 'bi-star' }}"></i>
                {{ $generation->is_favorite ? 'Saved' : 'Save' }}
            </button>
        </form>
        <a href="{{ route('generations.edit', $generation->id) }}" class="btn-alb-outline btn">
    ...
    <div id="originalCol" class="col-md-6" style="padding-right:2px;">
        <div style="background:#F9FAFB;border:1.5px solid #E5E7EB;border-radius:14px 0 0 14px;height:100%;padding:0;">
            <div style="background:#374151;color:white;padding:14px 20px;border-radius:12px 0 0 0;display:flex;align-items:center;gap:8px;">
                ...
                <span style="background:rgba(255,255,255,0.15);font-size:11px;padding:2px 8px;border-radius:20px;margin-left:auto;">{{ $import->original_brand }}</span>
                @endif
            </div>
            <div style="padding:20px;">
                @include('listings.partials.listing-column', [
...
<!-- A+ Content -->
@if($generation->generated_aplus_content)
<div class="alb-card mt-4 fade-in-up">
    <div class="alb-card-header">
        ...
        <button onclick="copyText('{{ addslashes($generation->generated_aplus_content) }}')" class="btn-alb-outline btn" style="font-size:12px;padding:6px 14px;">
            <i class="bi bi-copy me-1"></i>Copy
        </button>
    </div>
    <div style="background:#F5F3FF;border-radius:10px;padding:16px;font-size:13.5px;color:#374151;line-height:1.7;white-space:pre-wrap;">{{ $generation->generated_aplus_content }}</div>
</div>
@endif

<!-- Product Images -->
@php $productImages = $import->original_images ?? []; @endphp
@if(!empty($productImages))
<div class="alb-card mt-4 fade-in-up">
    <div class="alb-card-header">
        <h3 class="alb-card-title"><i class="bi bi-images me-2" style="color:#3B82F6;"></i>Product Images ({{ count($productImages) }})</h3>
        <div class="d-flex gap-2">
            <a href="{{ route('generations.images', $generation->id) }}" class="btn-alb-outline btn" style="font-size:12.5px;padding:7px 16px;">
                <i class="bi bi-grid me-1"></i>View Gallery
            </a>
            <a href="{{ route('export.images.zip', $generation->id) }}" class="btn-alb-primary btn" style="font-size:12.5px;padding:8px 18px;">
                <i class="bi bi-file-earmark-zip me-1"></i>Download All as ZIP
            </a>
        </div>
    </div>
    <p style="font-size:13px;color:#6B7280;margin-bottom:18px;">
        Original images scraped from the Amazon listing, renamed to <strong>{{ Str::slug($generation->brand_name ?: 'product') }}-{{ $import->asin ?? 'product' }}-NN</strong> format. Hover an image to download it individually.
    </p>
    <div class="row g-3">
        @foreach($productImages as $i => $imgUrl)
        <div class="col-6 col-md-3 col-lg-2">
            <div style="position:relative;border-radius:10px;overflow:hidden;border:1.5px solid #E5E7EB;aspect-ratio:1;background:#F9FAFB;" class="image-card">
                <img src="{{ $imgUrl }}" alt="Product image {{ $i + 1 }}" style="width:100%;height:100%;object-fit:cover;" onerror="this.parentElement.innerHTML='<div style=&quot;display:flex;align-items:center;justify-content:center;height:100%;color:#D1D5DB;font-size:24px;&quot;><i class=&quot;bi bi-image&quot;></i></div>'">
                <a href="{{ route('export.images.single', [$generation->id, $i]) }}" class="image-download-overlay" title="Download image {{ $i + 1 }}">
                    <i class="bi bi-download"></i>
                </a>
                <span style="position:absolute;top:6px;left:6px;background:rgba(0,0,0,0.6);color:white;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;">{{ $i + 1 }}</span>
            </div>
        </div>
        @endforeach
    </div>
...
<div class="alb-card mt-4 fade-in-up" id="exportPanel">
    ...
    <div class="row g-3">
        ...
        @endphp
        @foreach($formats as [$fmt, $label, $icon, $color, $desc, $sub])
        <div class="col-6 col-md-4 col-lg-2">
            <form method="POST" action="{{ route('export', $generation->id) }}">
                @csrf
                <input type="hidden" name="format" value="{{ $fmt }}">
                <button type="submit" style="width:100%;background:white;border:1.5px solid #E5E7EB;border-radius:12px;padding:16px 12px;cursor:pointer;text-align:center;transition:all 0.15s;" onmouseover="this.style.borderColor='{{ $color }}';this.style.background='#FAFAFA'" onmouseout="this.style.borderColor='#E5E7EB';this.style.background='white'">
                    <i class="{{ $icon }}" style="font-size:28px;color:{{ $color }};display:block;margin-bottom:8px;"></i>
                    <div style="font-size:12.5px;font-weight:700;color:#111827;margin-bottom:3px;">{{ $label }}</div>
        ...
        <div class="col-6 col-md-4 col-lg-2">
            <a href="{{ route('export.images.zip', $generation->id) }}" style="display:block;width:100%;background:white;border:1.5px solid #E5E7EB;border-radius:12px;padding:16px 12px;cursor:pointer;text-align:center;transition:all 0.15s;text-decoration:none;" onmouseover="this.style.borderColor='#3B82F6';this.style.background='#FAFAFA'" onmouseout="this.style.borderColor='#E5E7EB';this.style.background='white'">
                ...
                <div style="font-size:11px;color:#9CA3AF;">{{ count($import->original_images) }} image(s) packaged</div>
            </a>
        </div>
        @endif
    </div>
