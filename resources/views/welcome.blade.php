...
<nav>
    ...
    <div class="nav-cta">
        ...
        <a href="{{ route('register') }}" class="btn-nav-signup">Get Started Free →</a>
    </div>
</nav>
            ...
            <div class="row g-3">
                <div class="col-6">
                    <div style="background:#1a1a1a;border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:16px;height:100%;">
                        ...
                        <div style="margin-top:10px;display:flex;flex-direction:column;gap:5px;">
                            @foreach(['Keeps drinks cold for 24 hours or hot for 12 hours','Premium 18/8 stainless steel construction','BPA-free and eco-friendly materials'] as $b)
                            <div style="font-size:11px;color:rgba(255,255,255,0.4);display:flex;gap:6px;"><span style="color:rgba(255,255,255,0.2);">•</span>{{ $b }}</div>
                            @endforeach
                        </div>
                    </div>
                </div>
                <div class="col-6">
                    <div style="background:rgba(227,24,55,0.08);border:1px solid rgba(227,24,55,0.3);border-radius:10px;padding:16px;height:100%;position:relative;overflow:hidden;">
                        ...
                        <div style="margin-top:10px;display:flex;flex-direction:column;gap:5px;">
                            @foreach(['ADVANCED INSULATION: Triple-wall vacuum technology maintains beverage temperature','PREMIUM BUILD: Food-grade 18/8 stainless steel, completely BPA-free','ECO-CONSCIOUS: Reusable design reduces single-use plastic waste'] as $b)
                            <div style="font-size:11px;color:rgba(255,255,255,0.7);display:flex;gap:6px;"><span style="color:var(--red);">✓</span>{{ $b }}</div>
                            @endforeach
                        </div>
                    </div>
                </div>
        ...
        <div class="row g-4 mt-5 text-start">
            ...
            <div class="col-md-6 col-lg-3">
                <div class="step-card">
                    ...
                    <i class="bi {{ $icon }}" style="font-size:28px;color:var(--red);margin-bottom:14px;display:block;"></i>
        ...
        <div class="row g-4 mt-5 text-start">
            ...
            <div class="col-md-6 col-lg-3">
                <div class="feature-card">
                    ...
                    <div style="font-family:'Sora',sans-serif;font-size:15px;font-weight:700;margin-bottom:8px;">{{ $title }}</div>
...
<section class="section-dark text-center" id="pricing">
    <div class="container">
        ...
        <div class="row g-4 mt-5 justify-content-center">
            ...
            @endphp
            @foreach($pricingPlans as [$name, $price, $per, $feat1, $feat2, $feat3, $featured])
            <div class="col-md-4 col-lg-3">
                <div class="pricing-card {{ $featured ? 'featured' : '' }}">
                    ...
                    @endif
                    <div style="font-family:'Sora',sans-serif;font-size:17px;font-weight:800;margin-bottom:6px;">{{ $name }}</div>
                    <div style="display:flex;align-items:baseline;gap:4px;margin-bottom:20px;">
                        <span style="font-family:'Sora',sans-serif;font-size:36px;font-weight:900;">{{ $price }}</span>
                        <span style="color:rgba(255,255,255,0.4);font-size:13px;">{{ $per }}</span>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:24px;">
                        @foreach([$feat1, $feat2, $feat3] as $f)
                        <div style="display:flex;align-items:center;gap:8px;font-size:13px;color:rgba(255,255,255,0.6);">
                            <i class="bi bi-check-circle-fill" style="color:var(--red);font-size:14px;flex-shrink:0;"></i>{{ $f }}
                        </div>
                        @endforeach
                    </div>
                    <a href="{{ route('register') }}" style="display:block;text-align:center;padding:12px;border-radius:9px;font-size:14px;font-weight:700;text-decoration:none;background:{{ $featured ? 'var(--red)' : 'rgba(255,255,255,0.07)' }};color:white;border:1px solid {{ $featured ? 'transparent' : 'rgba(255,255,255,0.1)' }};transition:all 0.15s;" onmouseover="this.style.opacity='0.85'" onmouseout="this.style.opacity='1'">
                        Get Started
                    </a>
                </div>
            </div>
        ...
        <div style="margin-top:32px;">
            <a href="{{ route('billing.plans') }}" style="color:rgba(255,255,255,0.4);font-size:14px;text-decoration:none;">View full pricing comparison →</a>
        </div>
    </div>
</section>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-2 mb-4">
                ...
                @foreach(['Features', 'Pricing', 'How It Works', 'API Docs'] as $link)
                <div style="margin-bottom:8px;"><a href="#" style="font-size:13.5px;color:rgba(255,255,255,0.45);text-decoration:none;transition:color 0.15s;" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(255,255,255,0.45)'">{{ $link }}</a></div>
                @endforeach
            </div>
            <div class="col-md-2 mb-4">
                ...
                @foreach(['Sign In', 'Register', 'Dashboard', 'Billing'] as $link)
                <div style="margin-bottom:8px;"><a href="#" style="font-size:13.5px;color:rgba(255,255,255,0.45);text-decoration:none;transition:color 0.15s;" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(255,255,255,0.45)'">{{ $link }}</a></div>
                @endforeach
            </div>
            <div class="col-md-4 mb-4">
                ...
                @foreach(['Flipkart Generator', 'eBay Listing Builder', 'WooCommerce Import', 'Bulk URL Processing', 'Agency Dashboard'] as $f)
                <div style="margin-bottom:6px;display:flex;align-items:center;gap:8px;">
                    <span style="width:6px;height:6px;border-radius:50%;background:var(--red);display:inline-block;flex-shrink:0;"></span>
                    <span style="font-size:13px;color:rgba(255,255,255,0.35);">{{ $f }}</span>
                </div>
                @endforeach
            </div>
        </div>
